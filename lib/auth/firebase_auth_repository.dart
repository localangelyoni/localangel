...
class FirebaseAuthRepository implements AuthRepository {
  ...
  Future<UserCredential> signInWithGoogle() async {
    ...
    if (googleUser == null) {
      throw FirebaseAuthException(code: 'canceled', message: 'User canceled Google Sign-In');
    }
  ...
  Future<UserCredential> signInWithMicrosoft() async {
    throw FirebaseAuthException(code: 'provider-unavailable', message: 'Microsoft login not configured');
  }
  ...
  Future<UserCredential> signInWithFacebook() async {
    try {
      ...
      if (result.accessToken == null) {
        throw FirebaseAuthException(code: 'canceled', message: 'User canceled Facebook Login');
      }
      final credential = FacebookAuthProvider.credential(result.accessToken!.tokenString);
      return _auth.signInWithCredential(credential);
    } catch (e) {
      throw FirebaseAuthException(code: 'facebook-unavailable', message: 'Facebook login not available: $e');
    }
  }
  ...
  Future<void> linkProvider(SocialProvider provider) async {
    ...
    switch (provider) {
      case SocialProvider.google:
        final google = await GoogleSignIn(scopes: ['email', 'profile']).signIn();
        if (google == null) throw FirebaseAuthException(code: 'canceled', message: 'Canceled');
        final ga = await google.authentication;
        credential = GoogleAuthProvider.credential(idToken: ga.idToken, accessToken: ga.accessToken);
        break;
      case SocialProvider.apple:
        final rawNonce = _generateNonce();
        final nonce = _sha256ofString(rawNonce);
        final apple = await SignInWithApple.getAppleIDCredential(scopes: [AppleIDAuthorizationScopes.email], nonce: nonce);
      ...
      case SocialProvider.microsoft:
        throw FirebaseAuthException(code: 'provider-unavailable', message: 'Microsoft link not configured');
      ...
      case SocialProvider.facebook:
        ...
        credential = FacebookAuthProvider.credential(res.accessToken!.tokenString);
        break;
    }
